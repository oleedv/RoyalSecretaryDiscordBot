...
import { saveProspectMessage, closeProspect } from '../services/prospect/prospectService.js';
import { captureStaffMessage, flushThenDelete } from '../services/channelTranscript/channelTranscript.js';
...
export async function handleGuild(message, prospect) {
  ...
  if (cmd?.type === 'close') {
    ...
    await flushThenDelete(message.channel);
  ...
  if (cmd?.type === 'reply') {
    ...
    await saveProspectMessage(prospect.id, message.author.id, message.author.tag, replyContent, attachments, true, null, logMsg?.id, {
  ...
  await captureStaffMessage(message, { kind: 'prospect', record: prospect });
  return true;
}
