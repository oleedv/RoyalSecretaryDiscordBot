...
export async function getClosedTicketsByUser(userId) {
  return await query(
...
export async function createTicket(userId, guild, { steamId, reason } = {}) {
  ...
  const previousTickets = await getClosedTicketsByUser(userId);
...
export async function escalateTicket(ticket, tier, channel) {
  ...
  const infoEmbed = buildTicketInfoEmbed(userTag, ticket.user_id, ticket.uuid, tier);
